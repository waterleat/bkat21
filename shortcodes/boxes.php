...
function theme_shortcode_framed_box($atts, $content = null, $code) {
	...
	if($minheight){
		if(is_numeric($minheight)){$minheight=$minheight.'px';}
...
function theme_shortcode_note($atts, $content = null) {
	...
	return '<div class="'.apply_filters( 'theme_css_class', 'note' ).$align.$class.'"'.$width.'>'.$title.'<div class="'.apply_filters( 'theme_css_class', 'note_content' ).'">' . do_shortcode($content) . '<div class="'.apply_filters( 'theme_css_class', 'note_space' ).'"></div></div></div>';
}
}
