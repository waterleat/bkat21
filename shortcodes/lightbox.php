...
function theme_shortcode_lightbox($atts, $content = null, $code) {
	...
	if($imagesource_value){
		...
		<a title="'.$title.'" href="'.$href.'"'.($group?' data-fancybx-group="'.$group.'"':'').' class="lightbox image_size_'.$imagesize.($imageicon?' image_icon_'.$imageicon:'').'"'.$width.$height.$type.$close.$closeclick.$fittoview.$aspectratio.$autosize.$autowidth.$autoheight.'>' . $content . '</a>
		</div></div></figure>';
	} else {
		return '<a title="'.$title.'" href="'.$href.'"'.($group?' data-fancybx-group="'.$group.'"':'').' class="lightbox"'.$width.$height.$type.$close.$closeclick.$fittoview.$aspectratio.$autosize.$autowidth.$autoheight.'>'.$content.'</a>';
	}
}
}
