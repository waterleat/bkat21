...
function theme_shortcode_contactform($atts,$content = null) {
	...
	$gdpr_consent = wpml_t(THEME_NAME, 'GDPR Consent',stripslashes(theme_get_option('general','gdpr_consent')));
	if (!empty($gdpr_consent)){
		$gdpr_consent= '<p><input id="contact_'.$id.'_consent" name="contact_'.$id.'_consent" tabindex="8" type="checkbox" value="yes" />' .'<label for="contact_'.$id.'_consent">' .$gdpr_consent. '</label></p>';
		$tab_index='8';
	} else $tab_index='7';
	return <<<HTML
<div class="contact_form_wrap{$class}">
	...
	<form class="contact_form" action="{$action}" method="post" novalidate="novalidate">
		...
		{$gdpr_consent}
		<p><button {$bgcolor} type="submit" class="{$button_class} white" tabindex="{$tab_index}"><span{$textcolor}>{$submit_str}</span></button></p>
...
function theme_shortcode_categories($atts){
	...
	if($dropdown === 'true'){
		...
		$dropdown_id = 'cat-'.md5(rand(6, 10000).time());
		...
		while ($r->have_posts()){
			...
			if($thumbnail!='false'){
				...
				if(is_array($list_image) && isset($list_image['value'])){
					...
					$list_image=wp_get_attachment_image_src($list_image,array($thumbnail_size,$thumbnail_size), true);
					$list_image= $list_image[0];
				}
				if (has_post_thumbnail() && empty($list_image)){
				...
				if (!empty($list_image)) {$default_thumbnail_image=$list_image;}
				if (!empty($default_thumbnail_image)) {
					$output .= '<a class="thumbnail" href="'.get_permalink().'" title="'.get_the_title().'">';
