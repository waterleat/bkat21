...
function theme_shortcode_dropcaps($atts, $content = null, $code) {
   if (!is_array($atts)) $atts=array();
	if (isset($atts['style'])){
		...
		$atts=array_merge(array('style'=> $code,),$atts);
	}
	$dropcap=theme_shortcode_dropcap($atts, $content, 'dropcap');
	return $dropcap;
}
}
...
function theme_shortcode_dropcap($atts, $content = null, $code) {
	...
	if ($bold=='true') $bold=' bold'; else $bold='';

	$dropcaptypes = array("dropcap1", "dropcap2", "dropcap3", "dropcap4");
	if (!in_array($style,$dropcaptypes)) $style='dropcap1';	

	if (!is_numeric($lettertop) && !empty($lettertop)) $lettertop=preg_replace("/[^0-9]/","",$lettertop);
	if (!is_numeric($letterleft) && !empty($letterleft)) $letterleft=preg_replace("/[^0-9]/","",$letterleft);

	$dropcapcode=preg_replace("/[^0-9]/","",$style);
	$dropcap_inner='dropcap'.$dropcapcode.'_inner';
	$dropcapstyles = array();
	$dropcapinnerstyles = array();

	if ($lettercolor) {
			$dropcapstyles["color"] ='color:'.$lettercolor;
			$dropcapstyles["border-color"] ='border-color:'.$lettercolor;
		}
	if (!empty($lettertop) && $lettertop!='-1' || $lettertop=='0'){
		$dropcapinnerstyles["top"]='top:'.$lettertop.'px';
	}
	if (!empty($letterleft) && $letterleft!='2' || $letterleft=='0'){
		$dropcapinnerstyles["left"]='left:'.$letterleft.'px';
	}
	
	if($color){
		$color = ' '.$color;
	}
	if(!empty($dropcapstyles)){
		$outerstyle = ' style="'. implode(';', $dropcapstyles).'"';
	}else{
		$outerstyle = '';
	}
	if(!empty($dropcapinnerstyles)){
		$innerstyle = ' style="'. implode(';', $dropcapinnerstyles).'"';
	}else{
		$innerstyle = '';
	}
	return'<div class="' . $style.$color.'"'.$outerstyle.'><p class="'.$dropcap_inner.$bold.'"'.$innerstyle.'>' . do_shortcode($content) . '</p></div>';
}
}
...
function theme_shortcode_progress($atts, $content = null, $code) {
	...
	if ($animations <0 || empty($animations) && $animations!='0') $animations=1;
	$dataanimations = ' data-animations="'.$animations.'"';
	if($type){
		$type = ' progress_'.$type;
	}
	if($size){
	...
	return '<div class="progress'.$type.$size.$class.'" data-meter="'.$percent.'"'.$dataanimations.$trackcolor.'><span class="progress-meter" style="'.$barcolor.$textcolor.'">'.$text.'</span></div>';
}
}
...
function theme_shortcode_milestone($atts) {
	...
	return '<div class="milestone'.$icon_class.$size_class.$class.'">'.$icon.'<div class="milestone_number"'.$number_color_style.' data-separator="'.$separator.'" data-decimals="'.$decimals.'" data-speed="'.$speed.'" data-from="'.$numberfrom.'" data-to="'.$number.'"'. $dataanimations.'>'.$numberfrom.'</div><div class="milestone_subject"'.$subject_color_style.'>'.$subject.'</div></div>';
}
}
